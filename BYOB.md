...
The [slsa-github-generator](https://github.com/zktx-io/slsa-github-generator-test) repository hosts the code for the BYOB framework maintained by the OpenSSF SLSA tooling team. There are two main components you will use for your integration, the SLSA Setup Action and the SLSA Reuseable Workflow.

#### SLSA Setup Action (SSA)

The [setup-generic](https://github.com/zktx-io/slsa-github-generator-test/blob/main/actions/delegator/setup-generic) Action is used to initialize the BYOB framework. It returns a so-called "SLSA token" which is used in later steps:

```yaml
- uses: zktx-io/slsa-github-generator-test/actions/delegator/setup-generic@v2.0.0
```
...
```yaml
- uses: zktx-io/slsa-github-generator-test/.github/workflow/delegator_generic_slsa3.yml@v2.0.0
  with:
...
[issue #358](https://github.com/zktx-io/slsa-github-generator-test/issues/358). If
you have an issue related to any other triggers please submit a
[new issue](https://github.com/zktx-io/slsa-github-generator-test/issues/new/choose).
...
slsa-run:
  ...
  uses: zktx-io/slsa-github-generator-test/.github/workflows/delegator_generic_slsa3.yml@v2.0.0
  with:
    slsa-token: ${{ needs.slsa-setup.outputs.slsa-token }}
  secrets:
...
The first thing to do is to use a "low permission SRW". The SRW we used in our original integration is [delegator_generic_slsa3.yml](https://github.com/zktx-io/slsa-github-generator-test/blob/main/.github/workflows/delegator_generic_slsa3.yml), which calls the TCA with the [permissions for pushing release assets and publishing packages](https://github.com/zktx-io/slsa-github-generator-test/blob/01daa2e7c2c1c28d3bfbc3882102aed5da60121f/.github/workflows/delegator_generic_slsa3.yml#L137-L140). In order to reduce the number of permissions the TCA is called with, we recommend you use [delegator_lowperms-generic_slsa3.yml](https://github.com/zktx-io/slsa-github-generator-test/blob/main/.github/workflows/delegator_lowperms-generic_slsa3.yml) instead. This workflow does _not_ give the TCA the dangerous permissions above, and [only gives it `contents: read`](https://github.com/zktx-io/slsa-github-generator-test/blob/01daa2e7c2c1c28d3bfbc3882102aed5da60121f/.github/workflows/delegator_lowperms-generic_slsa3.yml#L142-L143) for repository read access. To update your integration:
...
- Pin your depenencies by hash except the [delegator workflow](https://github.com/zktx-io/slsa-github-generator-test/tree/main#referencing-slsa-builders-and-generators), to avoid dependency confusion attacks and speed up incidence response.
